...
## Endpoints
| CRUD   | HTTP   | Endpoint              | Description                                       | User Story |
|--------|--------|-----------------------|---------------------------------------------------|------------|
| CREATE | POST   | /user/register        | Create a new user                                 | 1          |
| READ   | POST   | /user/login           | Log in a user and generate a JWT Token            |            |
| READ   | GET    | /user/profile         | Get the use's profile information                 |            |
| CREATE | POST   | /courses              | Create a new course (flashcard/quiz)              |            |
| READ   | GET    | /courses              | Get a list of all courses                         |            |
| READ   | GET    | /courses/:id          | Get a specific course                             |            |
| READ   | GET    | /quiz-questions       | Get a list of all quiz questions                  | 4, 5       |
| UPDATE | PUT    | /quiz-questions/:id   | Update a specific quiz questions                  |            |
| DELETE | DELETE | /quiz-questions/:id   | Delete a specific quiz questions                  |            |
| READ   | GET    | /leaderboard          | Get the leadboard with users points               | 2          |
| READ   | GET    | /previous-quizes      | Get a list of previous quizzes the user has taken |            |
| READ   | GET    | /previous-quizzes/:id | Get the details of a specific previous quiz       |            |

## Potential Functions 
**User Functions**
- createUser(userDetails) : Create a new user in the db
- getUserByID(userId) : Retrieve a user's information by their ID
- getUserByUsername(username) : retrive a user's information by their username
  (user validation functions) -> jwt token etc

**Course Functions**
- createCourse(userID, courseDetails) Create a new course associated with a user
- getCourseById(courseId): Retrieve a course's information by its ID.
- getCoursesByUserId(userId): Retrieve all courses associated with a user.
- updateCourse(courseId, updatedDetails): Update a course's information in the database.
